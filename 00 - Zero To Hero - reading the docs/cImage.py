"""
image.py
This module provides a simple interface to create a window,
load an image and experiment with image based algorithms.
Many of which require pixel-by-pixel manipulation.
This is a educational module, its not intended to replace
the excellent Python Image Library, in fact it uses PIL.

The module and its interface and some of the code were
inspired/copied by/from John Zelle's graphics.py
...
# Changes:
#   * Modify class name for base image to be AbstractImage
#   This way we don't have a lower case class name running around.
#
#   * We still don't expect people to create an AbstractImage but rather
#     create an image through FileImage, ListImage, or EmptyImage.
#   * Add ability to convert an image to a list
#   * Add save function to write an image back to disk.
#
# Version 1.2  November 2007
# Changes:
#  Modify the setPosition function to position the image by the
#  top left corner rather than the center.
#  Add the exitOnClick method to ImageWin.
#  Use this as the last call in the program to
#  avoid early exits when running from the command line,
#  and nasty hangs when running from within IDLE
#
...
#   Pixel's __getitem__ function now supports looping i.e.
#   for value in Pixel(0, 1, 2):
...
class ImageWin(tk.Canvas):
    ...
    def getMouse(self):
        """
            Wait for mouse click and return a tuple with x,y
            position in screen coordinates after
            the click
        """
        self.mouseX = None
        self.mouseY = None
...
class AbstractImage(object):
    ...
    1. From an image file such as gif, jpg, png, ppm  for example:
            i = image('fname.jpb)
    ...
    def __init__(self, fname=None, data=[], imobj=None, height=0, width=0):
        """
        An image can be created using any of the following keyword parameters.
        When image creation is complete the image will be an rgb image.
        fname:  A filename containing an image.  Can be jpg, gif, and others
        data:  a list of lists representing the image.
        This might be something you construct by reading an asii format
        ppm file, or an ascii art file and translate into rgb yourself.
        ...
        # if PIL is available then use the PIL functions
        # otherwise fall back to Tk
        if pilAvailable:
        ...
        # +3 accounts for the ~3 pixel border in Tk windows
        self.centerX = self.width / 2+3
    ...
    def loadTkImage(self, fname):
        ...
        if suffix not in ['.gif', '.ppm']:
            raise ValueError("Bad Image Type: %s : Without PIL,\
                only .gif or .ppm files are allowed" % suffix)
    ...
    def getTkPixel(self, x, y):
        """
            Get a pixel at the given x,y coordinate.
            The pixel is returned as an rgb color tuple
    ...
    def setTkPixel(self, x, y, pixel):
        """
            ...
            The color must be specified as an rgb tuple (r,g,b) where
    ...
    def setPosition(self, x, y):
        """
            Set the position in the window where the top left
    ...
    def draw(self, win):
        ...
        ig = self.getImage()
        # save a reference else Tk loses it...
        self.imageCache[self.imageId] = ig
        AbstractImage.imageId = AbstractImage.imageId + 1
        self.canvas = win
        self.id = self.canvas.create_image(self.centerX,
                                           self.centerY,
                                           image=ig)
        _imroot.update()
    ...
    def saveTk(self, fname=None, ftype='gif'):
        ...
        if suffix == "":
            ...
            fname = fname+suffix
        if suffix not in ['.gif', '.ppm']:
            raise ValueError("Without PIL, only .gif or .ppm \
                files are allowed")
        try:
